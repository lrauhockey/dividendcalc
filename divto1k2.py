...
def getURLs(word):
    ...
    if word.find(":") > 0 :
        ...
        URLGoog = URLgoog = "https://www.google.com/search?q=TSE%3A" + word + "+dividend+history"
        canada = True
        statusTxt = "In Canada - "
    elif word.find(".UN") > 0 :
        ...
        URLGoog = URLgoog = "https://www.google.com/search?q=NASDAQ%3A" + word + "+dividend+history"
        canada = False
        statusTxt = "In US - "
    return URLcnbc, URLdiv, URLinfo, URLgoog, canada, statusTxt;
...
def getDivHistoryInfo(URLdiv):
    try:
        ...
        print("Exception in the div history lookup:", e)
        statusTxt = e 
        return statusTxt, line, frequency, freq, df_len, False;
...
def getDivInfo(URLinfo):
    try:
        ...
        return v, line, frequency, freq, 0, False;
    except Exception as e:
        print("Exception if URL info: ", e)
        return e, line, frequency, freq, 0, False;

def getInfoGoog(URLgoog):
    line =""
    frequency = ""
    freq = 0 
    statusTxt = ""
    infofound = False
    try:
        page = requests.get(URLgoog)
        message = page.text
        html = BeautifulSoup(message, 'html.parser')
        dfs = pd.read_html(str(html))
        #print(print(f'Total tables: {len(dfs)}'))
        if len(dfs)>0:
            df = dfs[0]
            #print(df.head())
            if len(df) < 4:
                ...
                infofound = True
            for index, row in df_top.iterrows():
                #print(row['Ex/EFF DATE'], row['CASH AMOUNT'])
                line = line + row["Ex/EFF DATE"] + "," + row["CASH AMOUNT"] + ","
            statusTxt = "Found DivHist -"
            return statusTxt, line, frequency, freq, 0, infofound
        else:
            return statusTxt, line, frequency, freq, 0, False;
    except ValueError as v:
            return v, line, frequency, freq, 0, False;
    except Exception as e:
        print("Exception on Goog info:", e)
        return e, line, frequency, freq, 0, False;
...
for word in words:
    line = word
    if word in stock_list and debug == False:
        #print("found: ", word)
        #ignore cause we already have it on our list
        #for debugging
        #dummy = dummy + 1
        skip_list.append(word) 
    else:
        ana_list.append(word)
        statusTxtcnbc = "cnbc not started - "
        statusTxthist = "divhist not started - "
        statusTxtinfo = "divInfo not started - "
        statusTxtgoog = "divGoog not started - "

        URLcnbc, URLdiv, URLinfo, URLgoog, canada, statusTxt = getURLs(word)
        ...
        if divhistFound:
            ...
            if divhistFoundinfo:
                ...
                statusTxtinfo, lineinfo, frequencyinfo, freqinfo, df_leninfo, divgoogFoundinfo = getInfoGoog(URLgoog)
                if divgoogFoundinfo:
                    line = line + lineinfo + "0"
                else:
                    line = line + "History Not Found"
        #print(statusTxt, line, frequency, freq, df_len, divhistFound)
        #print(line)
        #sleep(5)
        print(line)
        sleep(5)
file1 = open(stockfile, "a")
if debug == False:
    for i in ana_list:
        file1.writelines(i + '\n')
    file1.close()
print("Analyzed stoeck lst")
print(ana_list)
print("The List of stocks already analyzed:/n")
print(skip_list)